...
var APIIPRuleService = function($http, ENV, messageModel) {
	...
	/**
	 * Evaluates the live IP rule set against a hypothetical request.
	 * Calls POST /api/5.0/api_ip_rules/evaluate.
	 *
	 * @param {{ip: string, method: string, path: string, authType: string}} params
	 * @returns {Promise<{allowed: boolean, matchedRule: string, message: string}>}
	 */
	this.evaluateIPRule = function(params) {
		return $http.post(ENV.api.unstable + 'api_ip_rules/evaluate', params).then(
			function(result) {
				return result.data;
			},
			function(err) {

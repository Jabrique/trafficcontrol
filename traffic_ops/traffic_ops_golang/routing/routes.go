...
func Routes(d ServerData) ([]Route, http.Handler, error) {
	...
	routes := []Route{
		...
		// Evaluate endpoint: lets admins test a {ip, method, path, authType} tuple against live rules.
		// Uses EvaluateHandler(d.IPRuleCache) closure — cache injected at route-registration time.
		{Version: api.Version{Major: 5, Minor: 0}, Method: http.MethodPost, Path: `api_ip_rules/evaluate/?$`, Handler: apiiprule.EvaluateHandler(d.IPRuleCache), RequiredPrivLevel: auth.PrivLevelAdmin, RequiredPermissions: []string{"API-IP-RULE:READ"}, Authenticated: Authenticated, Middlewares: nil, ID: 5026080320006},
